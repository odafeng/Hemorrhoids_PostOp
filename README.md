...
3. **AI 衛教** → `ai-chat` Edge Function（JWT + rate limit）→ OpenAI embedding → pgvector RAG 檢索 → Claude Haiku API（SSE streaming，含 top-3 衛教 context）
...
npm test                    # 117 unit tests + 14 E2E tests
```
